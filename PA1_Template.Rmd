...
```
So mean number of steps per day is **`r round(meansteps,0)`** and median number of steps per day is **`r mediansteps`** . 
...
The interval which contains the maximum number of steps is **`r steps_by_interval[which(steps_by_interval$DailySteps == max(steps_by_interval$DailySteps)), 1]`**.This is computed using the code above to calculate maxSteps.
...
So in total there are **`r sum(is.na(activityData$steps))`** missing values in the data set which comprises a total percentage of **`r round(sum(is.na(activityData$steps))/nrow(activityData)*100,2)`** of the data.
...
Of the revised data set with NA values replaced the mean number of steps per day is **`r format(round(newmean,0), scientific = FALSE)`** and median number of steps per day is **`r format(round(newmedian,0), scientific = FALSE)`**.


The mean value has increased from **`r round(meansteps,0)`** to **`r format(round(newmean,0), scientific = FALSE)`** while the median has increased from **`r mediansteps`** to **`r format(round(newmedian,0), scientific = FALSE)`**. Since the mean is equal to the median and the median is a non integer value, it implies that the method of replacing NAs has left the mean and median equal ie. median value is an average value, this makes intuative sense since earlier we found that 13.11% of the data was NAs. Also since the mean has increased this implied that the NA values occurred in intervals where typically the wearer of the tracker was more active.
